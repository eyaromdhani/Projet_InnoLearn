...
class StageCondidatureRepository extends ServiceEntityRepository
{
    ...
    public function countByRecruiter(\App\Entity\User $recruteur, ?string $statut = null): int
    {
        $qb = $this->createQueryBuilder('s')
            ->select('COUNT(s.id)')
            ->innerJoin('s.id_offre', 'o')
    ...
    public function searchByStatut(string $statut): array
    {
        return $this->createQueryBuilder('s')
    ...
    public function searchByOffre(int $idOffre): array
    {
        return $this->createQueryBuilder('s')
            ->where('s.id_offre = :idOffre')
            ->setParameter('idOffre', $idOffre)
            ->orderBy('s.date_publication', 'DESC')
            ->getQuery()
            ->getResult();
    ...
    public function filterByDatePublication(\DateTimeInterface $minDate, \DateTimeInterface $maxDate): array
    {
        return $this->createQueryBuilder('s')
            ->where('s.date_publication >= :minDate')
            ->andWhere('s.date_publication <= :maxDate')
            ->setParameter('minDate', $minDate)
            ->setParameter('maxDate', $maxDate)
            ->orderBy('s.date_publication', 'DESC')
            ->getQuery()
    ...
    {
        return $this->createQueryBuilder('s')
            ->orderBy('s.date_publication', 'ASC')
            ->getQuery()
            ->getResult();
    }
