<?php

namespace App\Controller\Admin;

use App\Entity\Event;
use App\Entity\InscritEvent;
use App\Form\EventType;
use App\Repository\EventRepository;
use App\Repository\InscritEventRepository;
use App\Enum\StatutEvenementEnum;
use Doctrine\ORM\EntityManagerInterface;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;

#[Route('/admin/event')]
class AdminEventController extends AbstractController
{
    #[Route('/', name: 'admin_event_index', methods: ['GET'])]
    public function index(EventRepository $eventRepository, InscritEventRepository $inscritEventRepository, EntityManagerInterface $entityManager): Response
    {
        // Auto-cleanup logic from external project
        $now = new \DateTime();
        $events = $eventRepository->findAll();

        foreach ($events as $event) {
            // Auto-delete past terminated events or cancelled ones if desired
            // The external project logic was:
            if (
                $event->getStatut() === StatutEvenementEnum::ANNULE ||
                ($event->getStatut() === StatutEvenementEnum::TERMINE && $event->getDateFin() < $now)
            ) {

                // Delete associated registrations first
                $registrations = $inscritEventRepository->findBy(['event' => $event]);
                foreach ($registrations as $registration) {
                    $entityManager->remove($registration);
                }

                $entityManager->remove($event);
            }
        }
        $entityManager->flush();

        return $this->render('admin/event/index.html.twig', [
            'events' => $eventRepository->findAllWithInscriptions(),
            'inscriptions' => $inscritEventRepository->findAllWithRelations(),
        ]);
    }

    #[Route('/new', name: 'admin_event_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        $event = new Event();
        $form = $this->createForm(EventType::class, $event);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->persist($event);
            $entityManager->flush();

            return $this->redirectToRoute('admin_event_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('admin/event/new.html.twig', [
            'event' => $event,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'admin_event_show', methods: ['GET'])]
    public function show(Event $event): Response
    {
        return $this->render('admin/event/show.html.twig', [
            'event' => $event,
        ]);
    }

    #[Route('/{id}/edit', name: 'admin_event_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, Event $event, EntityManagerInterface $entityManager, InscritEventRepository $inscritEventRepository): Response
    {
        $form = $this->createForm(EventType::class, $event);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            // Synchronization Logic from external project
            $status = $event->getStatut();
            $inscriptions = $inscritEventRepository->findBy(['event' => $event]);

            if ($status === StatutEvenementEnum::ANNULE) {
                foreach ($inscriptions as $ins) {
                    $ins->setStatus('Annulée');
                }
            } elseif ($status === StatutEvenementEnum::TERMINE) {
                foreach ($inscriptions as $ins) {
                    $ins->setStatus('Terminée');
                }
            }

            $entityManager->flush();

            return $this->redirectToRoute('admin_event_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('admin/event/edit.html.twig', [
            'event' => $event,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'admin_event_delete', methods: ['POST'])]
    public function delete(Request $request, Event $event, EntityManagerInterface $entityManager): Response
    {
        $token = $request->request->get('_token');
        if ($this->isCsrfTokenValid('delete' . $event->getId(), is_string($token) ? $token : null)) {
            $entityManager->remove($event);
            $entityManager->flush();
        }

        return $this->redirectToRoute('admin_event_index', [], Response::HTTP_SEE_OTHER);
    }

    #[Route('/inscription/{id}/delete', name: 'admin_event_delete_inscription', methods: ['POST'])]
    public function deleteInscription(Request $request, InscritEvent $inscritEvent, EntityManagerInterface $entityManager): Response
    {
        $token = $request->request->get('_token');
        if ($this->isCsrfTokenValid('delete' . $inscritEvent->getId(), is_string($token) ? $token : null)) {
            $entityManager->remove($inscritEvent);
            $entityManager->flush();
        }

        return $this->redirectToRoute('admin_event_index', [], Response::HTTP_SEE_OTHER);
    }

    #[Route('/inscription/{id}/confirm', name: 'admin_event_confirm_inscription', methods: ['POST'])]
    public function confirmInscription(Request $request, InscritEvent $inscritEvent, EntityManagerInterface $entityManager): Response
    {
        $token = $request->request->get('_token');
        if ($this->isCsrfTokenValid('confirm' . $inscritEvent->getId(), is_string($token) ? $token : null)) {
            $inscritEvent->setStatus('Confirmé');
            $entityManager->flush();
        }

        return $this->redirectToRoute('admin_event_index', [], Response::HTTP_SEE_OTHER);
    }
}
