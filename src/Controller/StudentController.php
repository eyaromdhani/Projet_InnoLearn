...
=======
    ...
    #[Route('/projects', name: 'app_student_projects')]
    public function projects(ProjectRepository $projectRepository, DepotRepository $depotRepository, GeminiService $gemini): Response
    {
        ...
        foreach ($projects as $project) {
            ...
            if (!$difficulty) {
                $difficulty = $gemini->analyzeComplexity($title, $project->getDescription());
                // Note: We don't persist here to keep it dynamic, but it could be persisted for performance.
            }
    ...
    public function exportProjectsPdf(ProjectRepository $projectRepository, DompdfFactoryInterface $factory, GeminiService $gemini): Response
    {
        $projects = $projectRepository->findAll();
        $user = $this->getUser();
        ...
        foreach ($projects as $project) {
            ...
            // Use AI for complexity if not set
            if (!$project->getDifficulty()) {
                $project->setDifficulty($gemini->analyzeComplexity($project->getTitle(), $project->getDescription()));
            }
    ...
    public function projectDetail(int $id, ProjectRepository $projectRepository, DepotRepository $depotRepository, GeminiService $gemini): Response
    {
        ...
        $difficulty = $project->getDifficulty();
        if (!$difficulty) {
            $difficulty = $gemini->analyzeComplexity($project->getTitle(), $project->getDescription());
        }
        ...
        if ($form->isSubmitted()) {
            ...
            if (count($errors) === 0 && count($fileErrors) === 0 && $form->isValid()) {
                ...
                try {
                    ...
                    // Log error or notify admin silently if needed
                    // For now, we just avoid crashing the app
                }
...
=======
    ...
    public function downloadTicket(InscritEvent $inscription, PdfService $pdfService): Response
    {
        // Security check: ensure the registration belongs to the current user or its email
        $user = $this->getUser();
        ...
        $pdfContent = $pdfService->generateEventTicket($inscription);
